...
function AppContent() {
  ...
  useEffect(() => {
    ...
    if (path.includes("reset-password") || window.location.search.includes("token=")) {
      ...
      if (pId && Array.isArray(products) && products.length > 0) {
        const found = products.find((p) => String(p.id || p._id) === String(pId));
  ...
  // Synchronize selecting a product anywhere with full-screen Product Detail View
  useEffect(() => {
    if (selectedProduct) {
      setActivePage("product-detail");
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#FAF8F5] dark:bg-[#0B0D0E] text-[#141210] dark:text-[#F4EFE6] transition-colors duration-300 relative">
      ...
      {/* Floating AI Assistant Button: VISIBLE when Chatbot is closed, HIDDEN when open */}
      {!isChatbotOpen && (
        <button
